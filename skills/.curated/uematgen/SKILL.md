...
- Keep generated HLSL self-contained and deterministic. Name inputs clearly, for example `UV`, `Time`, `Speed`, `Tint`, `StyleTexture`, or `SdfScale`.
...
- If Unreal Python cannot connect a material property reliably, still generate a correct, meaningful Custom output pin and log that the user may wire it manually. Do not add fallback nodes that obscure or duplicate the intended output.
